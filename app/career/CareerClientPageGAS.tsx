...
const GOOGLE_APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbxZTpLsHJc-nWIaDNpXrZ62ynl6Fb7kf8KB9-XN9b1ZAa5nZUa593axuA-2-quMo6g/exec";

export default function CareerClientPageGAS() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setCvFile(file);
      setSubmitMessage("File selected: " + file.name);
      setIsSuccess(true);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add file data if CV was uploaded
      if (cvFile) {
        setSubmitMessage("Uploading file and submitting application...");
        const fileBase64 = await fileToBase64(cvFile);
        submitData.fileData = fileBase64;
        submitData.fileName = cvFile.name;
      }

      // Try fetch with no-cors mode
